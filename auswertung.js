...
// UPDATE VERBLEIB: MERKT SICH DIE VORHERIGE AUSWAHL DES ANGLERS
function updateVerbleibOptions(modus) {
    const verbleibSelect = document.getElementById('verbleib');
    const bisherigeAuswahl = verbleibSelect.value; // Wert vorher merken!
    ...
    // Wenn der vorher ausgewählte Wert in den neuen Optionen existiert, wieder herstellen!
    if (bisherigeAuswahl) {
        for (let i = 0; i < verbleibSelect.options.length; i++) {
            if (verbleibSelect.options[i].value === bisherigeAuswahl) {
